...
function bmiCalc_11(height, weight) {
    return weight / (height*height);
}

function bmiCalcResult_11(height, weight) {
    let bmi = bmiCalc_11(height, weight).toFixed(2);
    console.log(`For (h/w) = (${height}/${weight}), the BMI = ${bmi}`);
}

//bmiCalcResult_11(1.75, 55);
//bmiCalcResult_11(1.75, 70);
//bmiCalcResult_11(1.75, 85);
...
function bmiCalcSuggest_11(height, weight) {
    let bmi = bmiCalc_11(height, weight).toFixed(2);
    if (bmi < 18.5) {
...
//bmiCalcSuggest_11(1.75, 55);
//bmiCalcSuggest_11(1.75, 70);
//bmiCalcSuggest_11(1.75, 85);
...
bmi_data_11.forEach((item) => {
    bmiCalcSuggest_11(item.height, item.weight);
});
...
bmi_data2_11.forEach((item) => {
    const bmi = bmiCalc_11(item.height, item.weight);
    if(bmi < 18.5) {
        lowerCount++;
    } else if(bmi <= 24) {
        normalCount++;
